...
def roof_detect():
    ...
    if polygon and isinstance(polygon, list) and len(polygon) >= 3:
        ...
        prompt = (
            '請自動辨識這張衛星圖像中最大屋頂的面積（平方米），' '並回傳一個 JSON 格式：{\"area\": 面積數字, \"polygon\": [多邊形座標陣列]}，' 'polygon 為屋頂輪廓的經緯度陣列。'
        )

    payload = {
        'contents': [{'parts': [{'text': prompt}, {'inlineData': {'mimeType': 'image/png', 'data': img_base64}}]}]
    }
